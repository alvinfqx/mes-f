var viewModel = function () {
    ...
    this.unSelected = function () {
        $('#buttonsTable').find("tr").each(function () {
            ...
            viewModel: function () {
                ...
                $("#addSave").click(function () {
                    ...
                    if (addSelf.form.Sequence() && !/^[0-9]*$/.test(addSelf.form.Sequence())) {
                        msg.error("提示", "排序，请输入≥0的数字！");
                        return;
                    }

                    mf.ajax({
                        type: 'post',
                        url: '/MES/api/Menu/insertButtons',
    ...
    this.editClick = function () {
        ...
        var uniqueId = $('#buttonsTable').find(".selected").data("uniqueid");

        if (!uniqueId) {
            msg.info('提示', '请选择需要编辑的按钮！');
            return;
        }
        var selectContent = $("#buttonsTable").bootstrapTable('getRowByUniqueId', uniqueId);
        ...
        mf.dialog('#EditDialog', {
            viewModel: function () {
                ...
                $("#editSave").click(function () {
                    ...
                    mf.ajax({
                        type: 'post',
                        url: '/MES/api/Menu/updateButtons',
    ...
    this.deleteClick = function () {
        ...
        if (!uniqueId) {
            msg.info('提示', '请选择需要删除的按钮！');
            return;
        }
        var selectContent = $("#buttonsTable").bootstrapTable('getRowByUniqueId', uniqueId);

        msg.warning('警告', '确认要删除选中的按钮吗？', function () {
            //提交要删除的参数类别ID
            mf.ajax({
                type: 'post',
                url: '/MES/api/Menu/deleteButtons',
